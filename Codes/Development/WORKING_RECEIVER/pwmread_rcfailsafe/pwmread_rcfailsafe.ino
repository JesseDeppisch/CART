...
const int pwmPIN[] = {9}; // an array to identify the PWM input pins (the array can be any length)
...
float RC_decode(int CH) {
  ...
  // TODO - not calibrating
  //return PW[i];
  return CH_output;// TODO - NOT calibrating!
...
float calibrate(float Rx, int Min, int Mid, int Max) {
  ...
  return calibrated * 0.001; // This is why it's -1 to 1
}
